...
@section('title', 'Edit Template - Propszy')
@section('page_title', 'Edit Offer Letter Template')
        ...
        <div class="lg:col-span-8 glass-dark p-8 rounded-3xl border border-slate-850 shadow-2xl">
            <form action="{{ route('admin.templates.update', $template) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="subject" name="subject" required value="{{ old('subject', $template->subject) }}"
                        class="mt-2 block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-1 focus:ring-purple-500 text-sm">
                    @error('subject')
                        <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select id="type" name="type" required
                        ...
                        <option value="internal" {{ old('type', $template->type) === 'internal' ? 'selected' : '' }}>Internal Staff Layout (Dense corporate header)</option>
                        <option value="external" {{ old('type', $template->type) === 'external' ? 'selected' : '' }}>External Staff Layout (Premium border frame + elegant margins)</option>
                    </select>
                    @error('type')
                        <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <textarea id="content" name="content" rows="12" required
                        class="block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-1 focus:ring-purple-500 text-sm font-mono leading-relaxed">{{ old('content', $template->content) }}</textarea>
                    @error('content')
                        <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="pt-6 border-t border-slate-850 flex justify-end space-x-3">
                    ...
                    <button type="submit"
                        ...
                        <i class="fa-solid fa-save mr-2"></i> Update Template
                    </button>
                </div>
            </form>
        </div>
