...
@section('title', 'Edit Designation - RM HR Solutions')
@section('page_title', 'Edit Designation Details')
...
<div class="max-w-2xl mx-auto">
    <div class="glass-dark p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-xl">
        <h3 class="font-outfit font-bold text-xl text-slate-850 mb-6 flex items-center">
            <i class="fa-solid fa-id-card text-purple-650 mr-2.5"></i> Update Designation Details
        </h3>
        ...
        <form action="{{ route('admin.designations.update', $designation) }}" method="POST" class="space-y-6">
            ...
            <!-- Designation Name -->
            <div>
                <label for="name" class="block text-xs font-semibold uppercase tracking-wider text-slate-500">Designation Title *</label>
                <input type="text" id="name" name="name" required value="{{ old('name', $designation->name) }}"
            ...
            <div class="flex items-center justify-end space-x-3 pt-6 border-t border-slate-200">
                <a href="{{ route('admin.designations.index') }}" class="px-5 py-3 border border-slate-200 hover:border-slate-300 hover:bg-slate-50 rounded-xl text-xs font-semibold text-slate-650 transition-all">
                    Cancel & Return
                </a>
                <button type="submit" class="px-6 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white rounded-xl text-xs font-semibold transition-all shadow-lg shadow-purple-500/10">
                    <i class="fa-solid fa-circle-check mr-1.5"></i> Save Changes
                </button>
            </div>
        </form>
    </div>
