...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 glass-dark p-6 sm:p-8 rounded-3xl border border-slate-850 shadow-2xl space-y-6">
        <div>
        ...
        <form action="{{ route('admin.payslips.generate.submit') }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <!-- Employee -->
                <div>
                    <label for="employee_id" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">Select Staff Member</label>
                ...
                <!-- Format Style -->
                <div>
                    <label for="type" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">Layout Format Style</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 pt-4 border-t border-slate-850">
                <!-- Working Days -->
                <div>
                    <label for="working_days" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">Working Days</label>
                    <input type="number" id="working_days" name="working_days" required value="{{ old('working_days', 31) }}" oninput="recalculate()"
                        class="mt-2 block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-1 focus:ring-purple-500 text-sm">
                    @error('working_days') <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p> @enderror
                </div>

                <!-- Net Payable Days -->
                <div>
                    <label for="net_payable_days" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">Payable Days</label>
                    <input type="number" id="net_payable_days" name="net_payable_days" required value="{{ old('net_payable_days', 31) }}" oninput="recalculate()"
                        class="mt-2 block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-1 focus:ring-purple-500 text-sm">
                    @error('net_payable_days') <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p> @enderror
                </div>

                <!-- OT Days -->
                <div>
                    <label for="ot_days" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">OT Days</label>
                    <input type="number" id="ot_days" name="ot_days" required value="{{ old('ot_days', 0) }}" oninput="onOtDaysChange()"
                        class="mt-2 block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-1 focus:ring-purple-500 text-sm">
                    @error('ot_days') <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p> @enderror
                </div>

                <!-- Pay Mode -->
                <div>
                    <label for="pay_mode" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">Pay Mode</label>
                    ...
                    @error('pay_mode') <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p> @enderror
                </div>
            </div>

            <!-- 3. Earnings & Allowances Grid -->
            <div class="pt-4 border-t border-slate-850 space-y-4">
                <h4 class="text-xs font-bold text-emerald-400 uppercase tracking-wider"><i class="fa-solid fa-circle-arrow-up mr-2"></i>Earnings & Allowances</h4>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <!-- Basic Salary -->
                    <div>
                        ...
                        <input type="number" step="0.01" id="basic_salary" name="basic_salary" required value="{{ old('basic_salary', 0) }}" oninput="recalculate()"
                            class="mt-2 block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-1 focus:ring-purple-500 text-sm">
                        @error('basic_salary') <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p> @enderror
                    </div>

                    <!-- HRA -->
                    <div>
                        <label for="hra" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">H.R.A.</label>
                        <input type="number" step="0.01" id="hra" name="hra" required value="{{ old('hra', 0) }}" oninput="recalculate()"
                            class="mt-2 block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-1 focus:ring-purple-500 text-sm">
                        @error('hra') <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p> @enderror
                    </div>

                    <!-- Medical Allowance -->
                    <div>
                        <label for="medical_allowance" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">Medical Allowance</label>
                        <input type="number" step="0.01" id="medical_allowance" name="medical_allowance" required value="{{ old('medical_allowance', 0) }}" oninput="recalculate()"
                            class="mt-2 block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-1 focus:ring-purple-500 text-sm">
                        @error('medical_allowance') <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p> @enderror
                    </div>

                    <!-- Special Allowance -->
                    <div>
                        <label for="special_allowance" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">Special Allowance</label>
                        <input type="number" step="0.01" id="special_allowance" name="special_allowance" required value="{{ old('special_allowance', 0) }}" oninput="recalculate()"
                            class="mt-2 block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-1 focus:ring-purple-500 text-sm">
                        @error('special_allowance') <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p> @enderror
                    </div>

                    <!-- Leave Encashment -->
                    <div>
                        <label for="leave_encashment" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">Leave Encashment</label>
                        <input type="number" step="0.01" id="leave_encashment" name="leave_encashment" required value="{{ old('leave_encashment', 0) }}" oninput="recalculate()"
                            class="mt-2 block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-1 focus:ring-purple-500 text-sm">
                        @error('leave_encashment') <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p> @enderror
                    </div>

                    <!-- OT Allowance -->
                    <div>
                        <label for="ot_allowance" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">OT Allowance</label>
                        <input type="number" step="0.01" id="ot_allowance" name="ot_allowance" required value="{{ old('ot_allowance', 0) }}" oninput="recalculate()"
                            class="mt-2 block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-1 focus:ring-purple-500 text-sm">
                        @error('ot_allowance') <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p> @enderror
                    </div>
                </div>
            </div>
            ...
            <!-- 4. Deductions Grid -->
            <div class="pt-4 border-t border-slate-850 space-y-4">
                <h4 class="text-xs font-bold text-rose-400 uppercase tracking-wider"><i class="fa-solid fa-circle-arrow-down mr-2"></i>Deductions</h4>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <!-- Professional Tax -->
                    <div>
                        <label for="professional_tax" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">Professional Tax</label>
                        <input type="number" step="0.01" id="professional_tax" name="professional_tax" required value="{{ old('professional_tax', 0) }}" oninput="recalculate()"
                            class="mt-2 block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-1 focus:ring-purple-500 text-sm">
                        @error('professional_tax') <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p> @enderror
                    </div>

                    <!-- Provident Fund -->
                    <div>
                        <label for="provident_fund" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">Provident Fund (PF)</label>
                        <input type="number" step="0.01" id="provident_fund" name="provident_fund" required value="{{ old('provident_fund', 0) }}" oninput="recalculate()"
                            class="mt-2 block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-1 focus:ring-purple-500 text-sm">
                        @error('provident_fund') <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p> @enderror
                    </div>

                    <!-- ESIC -->
                    <div>
                        <label for="esic" class="block text-xs font-semibold uppercase tracking-wider text-slate-400">ESIC</label>
                        <input type="number" step="0.01" id="esic" name="esic" required value="{{ old('esic', 0) }}" oninput="recalculate()"
                            class="mt-2 block w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-xl text-slate-200 focus:outline-none focus:ring-1 focus:ring-purple-500 text-sm">
                        @error('esic') <p class="mt-1.5 text-xs text-rose-400">{{ $message }}</p> @enderror
                    </div>
                </div>
            </div>

            <!-- Live Net Payout Summary Block -->
            <div class="p-6 rounded-2xl bg-slate-950/80 border border-slate-850 flex flex-col md:flex-row items-center justify-between gap-4">
                <div class="grid grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-2 text-center md:text-left">
                    <div>
                        <span class="text-xxs font-bold text-slate-450 uppercase tracking-widest block">Gross Earnings</span>
                        <span id="label_gross" class="text-md font-bold text-emerald-400">₹0.00</span>
                    </div>
                    <div>
                        <span class="text-xxs font-bold text-slate-450 uppercase tracking-widest block">Total Deductions</span>
                        <span id="label_deductions" class="text-md font-bold text-rose-400">₹0.00</span>
                    </div>
                    <div class="col-span-2 md:col-span-1">
                        <span class="text-xxs font-bold text-slate-450 uppercase tracking-widest block">Net Payout</span>
                        <span id="label_net" class="text-lg font-black text-purple-400">₹0.00</span>
                    </div>
                </div>
    ...
    <div class="glass-dark p-6 sm:p-8 rounded-3xl border border-slate-850 shadow-2xl flex flex-col justify-between h-fit space-y-6">
        <div>
            <h3 class="font-outfit font-bold text-xl text-white mb-2 flex items-center">
            ...
            <div class="p-4 rounded-xl bg-slate-950/40 border border-slate-800 space-y-3">
                <span class="block font-semibold text-white text-xs"><i class="fa-solid fa-circle-info text-purple-400 mr-1.5"></i>CSV File Structure</span>
                <p class="text-[11px] text-slate-400 leading-relaxed">
                    The CSV table must contain the columns corresponding to all details:
                </p>
                <code class="block p-2 rounded bg-black/60 text-[10px] text-purple-300 font-mono select-all break-all leading-normal">
        ...
        <div class="space-y-4">
            <a href="{{ route('admin.payslips.template') }}"
                class="block w-full text-center px-4 py-3 bg-slate-850 hover:bg-slate-800 border border-slate-800 text-slate-200 rounded-xl text-xs font-bold transition-all">
            ...
            <form action="{{ route('admin.payslips.bulk') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    <label for="csv_file_payslip" class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Upload CSV Data Sheet</label>
                    <input type="file" id="csv_file_payslip" name="csv_file" accept=".csv" required
                        class="block w-full text-sm text-slate-450 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-pink-650 file:text-white hover:file:bg-pink-500 cursor-pointer">
                </div>
